# Prisma Schema Explorer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A regenerable, single-file HTML explorer for the 212-model Prisma schema, produced by `npm run schema:html`.

**Architecture:** A payload-extraction module parses `prisma/schema.prisma` with Prisma's own parser (`getDMMF` from `@prisma/internals`) into a compact JSON payload; a generator CLI injects that payload into an inline HTML/CSS/JS template and writes `prisma/schema-explorer.html`. The page is vanilla JS: schema-grouped sidebar with fuzzy search, per-model fields table, 1-hop SVG relation diagram, hash-based navigation.

**Tech Stack:** Node 18+ ESM, `@prisma/internals@6`, `node:test` for tests. No frontend framework, no CDN.

## Global Constraints

- Everything lives under `db/` (`/Users/harshitsan/Documents/heliverse/hcm/db`); paths below are relative to it.
- Output HTML must be fully self-contained: inline CSS/JS, no external requests, must work from `file://`.
- Prisma stays at major version 6 (`prisma@6` / `@prisma/client@6` already installed); `@prisma/internals` must also be `@6`.
- The generator must exit non-zero with the Prisma error if the schema fails to parse.
- Page template JS must not contain backticks or `${` sequences (it lives inside a JS template literal in the generator).
- The payload JSON must have `<` escaped as `<` before injection (schema doc comments could otherwise break out of the script tag).
- Spec: `docs/superpowers/specs/2026-07-09-prisma-schema-explorer-design.md`.

---

### Task 1: Payload extraction module

**Files:**
- Create: `db/scripts/schema-payload.mjs`
- Test: `db/scripts/schema-payload.test.mjs`
- Modify: `db/package.json` (add devDependency via npm install)

**Interfaces:**
- Consumes: `prisma/schema.prisma` (already valid — `npx prisma validate` passes).
- Produces: `extractPayload(schemaPath: string): Promise<Payload>` where `Payload = { modelCount: number, schemas: Array<{name, models: string[]}>, models: Record<string, Model>, edges: Array<{from, to, label}> }` and `Model = { name, pgSchema, tableName, doc, fields: Array<{name, col, type, kind, list, opt, pk, unique, fk: {target, from?, to?}|null, def, doc}> }`. Task 2 relies on exactly these names.

- [ ] **Step 1: Install @prisma/internals**

```bash
cd /Users/harshitsan/Documents/heliverse/hcm/db
npm install --save-dev @prisma/internals@6
```

Expected: installs without ERESOLVE errors (same major as installed prisma@6).

- [ ] **Step 2: Write the failing test**

Create `db/scripts/schema-payload.test.mjs`:
...
Run: `node --test scripts/schema-payload.test.mjs`
Expected: FAIL — `Cannot find module '.../scripts/schema-payload.mjs'`
...
Run: `node --test scripts/schema-payload.test.mjs`
Expected: PASS, 5/5 tests. If `getDMMF` rejects the option shape, check `node_modules/@prisma/internals/dist/index.d.ts` for the `getDMMF` signature — it accepts `{ datamodel: string }` in v6.

- [ ] **Step 6: Commit**

```bash
cd /Users/harshitsan/Documents/heliverse/hcm
git add db/scripts/schema-payload.mjs db/scripts/schema-payload.test.mjs db/package.json db/package-lock.json
git commit -m "feat(db): extract schema-explorer payload from Prisma DMMF"
```

---

### Task 2: Generator CLI + HTML template, generate and open

**Files:**
- Create: `db/scripts/generate-schema-explorer.mjs`
- Modify: `db/package.json` (add `schema:html` script)
- Output (generated, committed): `db/prisma/schema-explorer.html`

**Interfaces:**
- Consumes: `extractPayload(schemaPath)` from `./schema-payload.mjs` (Task 1 shape).
- Produces: `db/prisma/schema-explorer.html`; npm script `schema:html`.

- [ ] **Step 1: Write the generator with inline template**

Create `db/scripts/generate-schema-explorer.mjs`. Note the Global Constraints: the `TEMPLATE` literal must contain no backticks and no `${` — the page JS therefore uses only string concatenation.

**File order matters:** imports first, then the `const TEMPLATE = ...` declaration (second code block below), then the run block (first code block below) — top-level code referencing `TEMPLATE` before its `const` would throw a TDZ ReferenceError.
...
The template — placed BETWEEN the imports and the try/catch run block (full content — copy verbatim):
...
In `db/package.json` add to `"scripts"`:

```json
"schema:html": "node scripts/generate-schema-explorer.mjs"
```

- [ ] **Step 3: Run the generator (this is the failing/passing cycle for a CLI)**

Run: `npm run schema:html`
Expected: `Wrote .../db/prisma/schema-explorer.html (212 models, ~900-1500 KB)`

- [ ] **Step 4: Verify the output programmatically**

```bash
grep -c '"modelCount":212' prisma/schema-explorer.html   # expect 1
grep -c 'ApiContractVersion' prisma/schema-explorer.html # expect >= 1
grep -c 'src=\|href="http\|@import' prisma/schema-explorer.html # expect 0 (self-contained)
node --test scripts/schema-payload.test.mjs               # still 5/5 PASS
```

- [ ] **Step 5: Verify failure mode**

Temporarily break the schema (e.g. append `model {` to a copy) and point the script at it — OR simpler: run `node -e "import('./scripts/schema-payload.mjs').then(m=>m.extractPayload('does-not-exist.prisma')).catch(e=>{console.error('ok, throws');process.exit(0)})"`.
Expected: generation path exits non-zero on bad input (the try/catch in the CLI handles this).

- [ ] **Step 6: Open in the browser and spot-check**

Run: `open prisma/schema-explorer.html`
Check: sidebar shows 15 groups; searching `emp` narrows; clicking a model shows fields matching `schema.prisma`; FK links and diagram boxes navigate; browser Back returns.

- [ ] **Step 7: Commit**

```bash
cd /Users/harshitsan/Documents/heliverse/hcm
git add db/scripts/generate-schema-explorer.mjs db/package.json db/prisma/schema-explorer.html
git commit -m "feat(db): single-file HTML schema explorer (npm run schema:html)"
```
