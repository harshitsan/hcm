...
/**
 * Email Templates step (FBG-28): view and edit the acknowledgement, status
 * change, approver reminder and coordinator escalation templates. Updates
 * apply to subsequent notifications only.
 */
export function ConfigTemplatesStep({ store }: { store: FeedbackConfigStore }) {
  const { config } = store
  const [activeId, setActiveId] = useState(config.templates[0]?.id ?? '')
  const active = config.templates.find((t) => t.id === activeId)
    ...
    <div className='grid w-full grid-cols-1 gap-4 xl:grid-cols-3'>
      <Card className='gap-3 border-none bg-white py-4'>
        <CardHeader className='px-4'>
          ...
          <p className='text-paragraph-sm text-neutral-1000'>
            Governed wording for every feedback/grievance notification event.
          </p>
        </CardHeader>
      ...
      <Card className='gap-3 border-none bg-white py-4 xl:col-span-2'>
        <CardHeader className='px-4'>
          ...
          <p className='text-paragraph-sm text-neutral-1000'>
            Placeholders like {'{{entry_id}}'} resolve at send time. Anonymous
            entries never expose the submitter's identity in notifications.
          </p>
        </CardHeader>
