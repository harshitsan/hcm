# SatelliteHR POC — Audit Gap Matrix (2026-07-11)

## 1. How this was produced

Two multi-agent audits over the frontend-only mock HRMS. **Code/feature audit:** 32 per-module auditors graded every module against the Kensium HRMS feature docs, Phase I/II BRDs, user stories and `design/00-CONVENTIONS.md`, after a baseline pass captured the canonical page anatomy (workflows and leave are the de facto reference implementations). **Live-app UX audit:** 8 screen-group auditors drove the running app with Playwright across Platform Admin, Company Admin and Employee roles, capturing 342 screenshots. Scoring legend: 1-5 per axis — feature completeness, layout conformance to the canonical anatomy, config findability. Payroll is out of scope; RBAC fixes are deferred per direction.
...
## 5. Merged prioritized worklist

Features first; demo order employees → workflows → lifecycle/leave/attendance/recruitment → rest. Demo-unblocking quick wins pulled forward. Overlapping code/UX findings merged (source references in parentheses: code = §3 worklist, UX = §4 worklist). Payroll excluded; RBAC deferred to the tail.
...
## 8. Appendix: canonical anatomy baseline
